...
function actionFormatter(value, row, index) {
    ...
    if (row.transStatus === 1) {
        return [
            '<a class="btn btn-danger btn-xs" id="stop" type="button"><i class="fa fa-stop" aria-hidden="true"></i>&nbsp;停止</a>'
        ].join('');
    } else if (row.transStatus === 2) {
        return [
            '<a class="btn btn-primary btn-xs" id="start" type="button"><i class="fa fa-play" aria-hidden="true"></i>&nbsp;启动</a>',
            '&nbsp;&nbsp;',
...
function bindButton() {
    ...
    // 启动全部停止的任务
    $('#startAll').delegate('#startAll','click',function(e) {
        layer.confirm(
            '确定启动全部已停止的转换？',
            {btn: ['确定', '取消']},
            function (index) {
    ...
    // 启动全部停止的任务
    $('#stopAll').delegate('#stopAll','click',function(e) {
        layer.confirm(
            '确定停止全部已启动的转换？',
            {btn: ['确定', '取消']},
            function (index) {
